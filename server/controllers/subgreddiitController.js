...
exports.subgreddiit_create_post_content = function (req, res, next) {
    ...
    let banned_keywords = req.body.banned_keywords;
    let found_banned_keyword = false;
    // Iterate over banned keywords 
    for (let i = 0; i < banned_keywords.length; i++) {
        // If banned keyword is found in post content
        while (post.content.includes(banned_keywords[i])) {
            // Replace banned keyword with asterisks
            post.content = post.content.replace(banned_keywords[i], '*'.repeat(banned_keywords[i].length));
            found_banned_keyword = true;
        }
    }

    // Save post and update subgreddiit.posts
    Post.create(post, (err, post) => {
        if (err) console.log(err);
            ...
            SubGreddiit.findOneAndUpdate(
                ...
                { $push: { posts: post._id } }, (err, subgreddiit) => {
                    if (err) console.log(err);
                    else {
                        console.log(subgreddiit)
                        console.log('Post added to subgreddiit');
                        // Return foundBannedKeyword
                        res.json({ foundBannedKeyword: found_banned_keyword });
                    }
                })
        }
    })
...
exports.subgreddiit_block_report = function (req, res, next) {
    ...
    SubGreddiit.findOne({ name: req.params.name }, (err, subgreddiit) => {
        if (err) console.log(err);
            ...
            if (subgreddiit.moderators.includes(req.user._id)
                && subgreddiit.common_members.includes(req.body.blocked_user)) {
                // Check if the user is already blocked
                if (!subgreddiit.blocked_members.includes(req.body.blocked_user)) {
                    // Add the user to the blocked_members list and banned members list
                    // Remove the user from the common_members list

                    SubGreddiit.findOneAndUpdate(
                        { name: req.params.name },
                        { $push: { blocked_members: req.body.blocked_user, banned_members: req.body.blocked_user } },
                        (err, subgreddiit) => {
                            if (err) console.log(err);
                            else {
                                ...
                                SubGreddiit.findOneAndUpdate(
                                    ...
                                    (err, subgreddiit) => {
                                        if (err) console.log(err);
                                            ...
                                            // Set the post.blocked true for every post in this sg posted by the blockeduser
                                            // Post.updateMany(
                                            //     { reported_by: req.body.blocked_user },
                                            //     { status: 'Blocked' },
                                            ...
                                            //         if (err) console.log(err);
                                            //         else {
                                            //             console.log('Posts status set to blocked');
                                            //             // Remove the user from the upvotes and downvotes of every post
                                            //             Post.updateMany(
                                            //                 { upvotes: req.body.blocked_user },
                                            //                 { $pull: { upvotes: req.body.blocked_user } },
                                            //                 (err, posts) => {
                                            //                     if (err) console.log(err);
                                            //                     else {
                                            //                         console.log('User removed from upvotes');
                                            //                         Post.updateMany(
                                            //                             { downvotes: req.body.blocked_user },
                                            //                             { $pull: { downvotes: req.body.blocked_user } },
                                            //                             (err, posts) => {
                                            //                                 if (err) console.log(err);
                                            //                                 else {
